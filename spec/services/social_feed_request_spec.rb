...
describe SocialFeedRequest do
  describe '#get' do
    subject { described_class.new.send(:get, platform, filter) }
    let(:client_response) { double(get: response) }

    before do
      expect_any_instance_of(SocialFeedClient).to receive(:call).once.and_return(client_response)
    end
